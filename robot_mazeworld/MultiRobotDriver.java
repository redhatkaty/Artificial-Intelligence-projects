...
import assignment_mazeworld.MultiRobotProblem.SimpleMazeNode;
...
public class MultiRobotDriver extends Application {
	...
	private void initMazeView() {
		//maze = Maze.readFromFile("eightpuzzles.maz");//changemaze
		maze = Maze.readFromFile("simple.maz");//maze
	...
	private void runSearches() {
		...
		int[] sx = {0,1,2,3};//4robots
		int[] sy = {0,0,0,0};
		int[] gx = {6,6,6,6};
		int[] gy = {3,2,1,0};
//		int[] sx = {2,1,1,2,0,2,0,0};//8puzzles
//		int[] sy = {0,2,0,2,1,1,2,0};
//		int[] gx = {1,2,0,1,2,0,1,2};
//		int[] gy = {2,2,1,1,1,0,0,0};
//		int[] sx = {39,0,38};//40_40maze
//		int[] sy = {0,0,3};
//		int[] gx = {21,22,23};
//		int[] gy = {20,20,20};
//		int[] sx = {2,1,0};//trickyproblem
//		int[] sy = {0,0,0};
//		int[] gx = {2,3,4};
//		int[] gy = {3,3,3};
...
//		//test
//		int[] sx1={0,0,0};
//		SimpleMazeProblem mazeProblem2 =new SimpleMazeProblem(maze, sx1, sy, gx,gy);
//		SearchNode testequal=mazeProblem.startNode;
//		if (mazeProblem.startNode.equals(mazeProblem2.startNode)) System.out.println("successors equals works" );
	...
	private class AnimationPath {
		private ArrayList<Node> piece=new ArrayList<Node>();
		...
		public AnimationPath(MazeView mazeView, List<SearchNode> path) {
			...
			int totalrobot=firstNode.getX().length;
			for(int i=0;i<totalrobot;i++)
				piece.add(mazeView.addPiece(firstNode.getX()[i], firstNode.getY()[i]));
...
//			lastY = firstNode.getY();
		...
		public void doNextMove() {
			...
			if (currentMove < searchPath.size() && animationDone) {
				SimpleMazeNode mazeNode = (SimpleMazeNode) searchPath
						.get(currentMove);
				int totalrobot=mazeNode.getX().length;
				int[] dx=new int[totalrobot]; 
				int[] dy=new int[totalrobot];
				for(int i=0;i<totalrobot;i++){
					dx[i]=mazeNode.getX()[i] - lastX[i];
					dy[i] = mazeNode.getY()[i] - lastY[i];
				}
		...
		public void animateMove(ArrayList<Node> n, int[] dx, int[] dy) {
			animationDone = false;
			for(int i=0;i<n.size();i++){
				TranslateTransition tt = new TranslateTransition(
						Duration.millis(300), n.get(i));
				tt.setByX(PIXELS_PER_SQUARE * dx[i]);
				tt.setByY(-PIXELS_PER_SQUARE * dy[i]);
