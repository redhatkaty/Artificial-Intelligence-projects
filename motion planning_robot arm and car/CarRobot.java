...
public class CarRobot {
	...
	protected double initial2[][] = {{4, 0.5}, {8, 0}, {4, -0.5}, {-4, -0.5}, {-4, 0.5}};
	...
	public double[][] getpath() {
		...
		for (int i = 0; i < rn.length; i++) {
			current[0] = initial2[i][0] * Math.cos(s.getTheta()) - initial2[i][1] * Math.sin(s.getTheta()) + s.getX();
			current[1] = initial2[i][0] * Math.sin(s.getTheta()) + initial2[i][1] * Math.cos(s.getTheta()) + s.getY();
			rn[i][0] = current[0];
			rn[i][1] = current[1];
		}
		return rn;
